...
function Write-SuspiciousCard {
    ...
    if ($Mod.Fullwidth -and $Mod.Fullwidth.Count -gt 0) {
        ...
        foreach ($fw in ($Mod.Fullwidth | Sort-Object)) {
            Write-Host "  │    " -ForegroundColor DarkRed -NoNewline
            Write-Host "FULLWIDTH: $fw" -ForegroundColor Red
        }
    }
...
Write-Host "🔍 Pass 1 — Hash verification (Modrinth + Megabase)..." -ForegroundColor Red
...
Write-Host "🔬 Pass 2 — Deep-scanning all $totalFiles $modWord..." -ForegroundColor Red
$idx = 0
...
Write-Host "🛡️  Pass 3 — Bypass/injection scan on all $totalFiles $modWord..." -ForegroundColor Red
$idx = 0
...
Write-Host "🔎 Pass 4 — Obfuscation analysis on all $totalFiles $modWord..." -ForegroundColor Red
$idx = 0
...
$jvmFlags = @()
Write-Host "⚡ Pass 5 — Scanning JVM for agents and injections..." -ForegroundColor Red
...
Write-Rule "━" 76 Red
Write-Host ""
Write-Host "  ✨ Analysis complete!" -ForegroundColor Red
Write-Host ""
